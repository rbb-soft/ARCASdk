<?php
/**
 * Emisión de Factura C (cbte_tipo=11) con warmup y retry + PDF.
 *
 * Igual que factura_c_basica.php pero, después de una emision exitosa,
 * genera el PDF del comprobante con QR oficial ARCA via
 * {@see \Rbbsoft\ArcaSdk\Pdf\ComprobantePdfGenerator}. El PDF se guarda
 * en `<raiz_del_proyecto>/comprobantes/{tipoCmp}-{nroCmp}.pdf`
 * (crea el directorio si no existe).
 *
 * Pensado para monotributistas que solo pueden emitir C en producción.
 * En homologación ARCA no valida régimen, así que este mismo script
 * sirve para el smoke test contra homo.
 *
 * Uso:
 *   php examples/factura_c_basica_pdf.php                    [warmup 600s, 3 intentos, 60s entre]
 *   php examples/factura_c_basica_pdf.php 0                  [sin warmup, 3 intentos, 60s entre]
 *   php examples/factura_c_basica_pdf.php 0 1 0              [sin warmup, 1 intento]
 *
 * Comportamiento:
 *   1. Warmup: espera $warmupSeconds (default 600s = 10 min, el lock
 *      del CEE de ARCA en homologación tras una emision reciente) para
 *      que ARCA libere el TA vigente. El lock dura 10 min en homo y
 *      2 min en prod; ajustar segun ambiente.
 ...
 *      Validation) no se reintentan: el script imprime la causa y
 *      termina.
 *   5. Tras una emision exitosa se genera el PDF. Si la generacion
 *      del PDF falla (faltan campos, error de mPDF, no se puede
 *      escribir el archivo, etc.) la excepcion RuntimeException del
 *      ComprobantePdfGenerator cae en el catch generico (exit 9) y
 *      el comprobante ya autorizado en ARCA queda sin PDF — se puede
 *      regenerar pasando el mismo array de merge a posteriori.
 *
 * Si todos los intentos fallan, el script imprime un resumen con la
 * causa raíz de cada uno y los external_id para inspección.
 */
...
/**
 * Espera $seconds con un countdown visible. Se puede interrumpir
 * con Ctrl+C y al volver a correr el script el warmup se respeta.
 */
...
$data = [
    'cbte_tipo'   => TiposComprobante::FACTURA_C,   // 11 — monotributo
...
for ($i = 1; $i <= $maxAttempts; $i++) {
    ...
    try {
        ...
        echo "\n  Padron A13 (preview; el wrapper actual no acepta Emisor):\n";
        $padronCasos = [
            'emisor'   => (int) $config->cuit,
            'receptor' => (int) $data['receptor_documento_nro'],
        ];
        foreach ($padronCasos as $rol => $cuit) {
            // Catch amplio: el padrón es informativo, no debe tumbar el PDF.
            // obtenerEmisor() puede tirar WsaaException si la autenticacion
            // interna falla, ademas de las excepciones propias del padron.
            try {
                $em = $arca->obtenerEmisor($cuit);
                $denom = $em->razonSocial ?? $em->apellidoNombre ?? '(sin denominacion)';
                printf("  - %-8s CUIT %-12s %s | cond IVA=%s",
                    $rol, $cuit, $denom, $em->condicionIva ?? '(s/d)');
                if ($em->categoriaMonotributo !== null) {
                    echo " | monotributo cat=" . $em->categoriaMonotributo;
                }
                echo "\n";
            } catch (\Throwable $e) {
                printf("  - %-8s CUIT %-12s %s\n", $rol, $cuit, get_class($e) . ': ' . $e->getMessage());
            }
        }
        $pdfPath = (new ComprobantePdfGenerator(__DIR__ . '/../comprobantes'))->generar($resp);
        echo "  PDF:             {$pdfPath}\n";
        echo "\nEmision exitosa.\n";
        exit(0);
    ...
    // Si llegamos aca, el intento fue transitorio. Si quedan reintentos, sleep.
    if ($i < $maxAttempts) {
