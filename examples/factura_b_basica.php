<?php
/**
 * Emisión de Factura B (cbte_tipo=6) con warmup y retry.
 *
 * La Factura B se emite desde un Responsable Inscripto hacia un
 * receptor que bajo RG 5616 puede ser CF, EX o NC (no RI, no MT).
 * Discrimina IVA en el comprobante.
 *
 * En homologacion ARCA NO valida el regimen del emisor: un monotributo
 * puede emitir Factura B sin problema (probado 2026-08-04 con CAE
 * real 86310728647866, receptor CF). En produccion, monotributo queda
 * rechazado con el codigo 10017. El RECEPTOR si esta validado en
 * homo: bajo RG 5616 la matriz `Cmp_Clase` exige receptor CF/EX/NC;
 * un receptor MT (monotributo) devuelve codigo 10243
 * ("Condicion IVA receptor no es valido para la clase de comprobante").
 * Por eso el ejemplo usa receptor CF, no MT.
 *
 * Uso:
 *   php examples/factura_b_basica.php                    [warmup 600s, 3 intentos, 60s entre]
 *   php examples/factura_b_basica.php 0                  [sin warmup, 3 intentos, 60s entre]
 *   php examples/factura_b_basica.php 0 1 0              [sin warmup, 1 intento]
 *
 * Comportamiento: ver factura_a_basica.php (mismo patron warmup+retry).
 */
...
$data = [
    'cbte_tipo'   => TiposComprobante::FACTURA_B,    // 6 - requiere emisor RI
    'concepto'    => 1,                               // 1 Productos
    'receptor_documento_tipo' => 80,                   // 80=CUIT
    'receptor_documento_nro'  => $cuitReceptor,
    'receptor_condicion_iva'  => 'CF',                  // receptor consumidor final (RG 5616: B no acepta MT)
    'mon_id'      => 'PES',
    'mon_cotiz'   => '1.00',
    'items' => [
